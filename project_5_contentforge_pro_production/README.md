...
## Architecture
FastAPI exposes a `POST /run-task` route that calls Project 4's existing 5-agent LangGraph graph directly — none of the agents themselves change. Every run is written to the database as `"pending"` and then updated with its result, and can be looked up later via `GET /runs/{id}`. The whole app is packaged in a Docker container with secrets injected only at runtime, and protected by rolling-window rate limiting. A pytest-based suite, including an LLM-judge check, must pass before a new prompt or setting version is allowed to go live, and a rollback function restores the last version that actually passed — not just whatever came before it.
