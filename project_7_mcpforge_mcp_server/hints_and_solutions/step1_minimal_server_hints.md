...
You need three small pieces: a config loader for `LOG_LEVEL` (reuse Doc01's, don't rewrite it), a `FastMCP` server object, and one function decorated as a tool that stores text in a plain Python list.

Things to use:
- `from mcp.server.fastmcp import FastMCP` — the server class.
- `mcp = FastMCP("notes-server")` — build it once, give it a name.
