...
Only 2 hints — work through them in order, and don't jump ahead until you've genuinely tried. Each hint has 3 depth levels: **Basic** (the plain idea), **Intermediate** (proper LangGraph), **Advanced** (how a real codebase would actually structure this, so Document 10 can build on it). Read Basic first even if you already know LangGraph — it's the fastest way to spot exactly what each deeper level adds.
...
**Difference between Basic, Intermediate, and Advanced:** Basic names the 4 files, their jobs, and the exact tools. Intermediate connects this task explicitly to the 2 exercises it's built from, and spells out each file's real content. Advanced asks the harder design question underneath the whole task — not "does the graph work once," but "is this skeleton actually safe for Document 10 to import and build on," which is the literal stated purpose of this Build Task — and answers it with a parameterized checkpointer and a real rejection path, not just an approval path.
...
nodes.py:
    think(state): call the model; decide "call_tool" or "finish"; append to scratchpad
    act(state): run the chosen tool; append the result to scratchpad
    risky_action(state): call interrupt() to pause for a human; once resumed, do the placeholder action
...
graph.py:
    function build_graph():
        ...
        start -> think
        think, conditional on route: "call_tool" -> act, "risky" -> risky_action, "finish" -> end
        act -> think  (loop back)
        risky_action -> end
...
test_graph.py:
    ...
    test 2: a task that reaches risky_action pauses (check __interrupt__ in the result)
    test 3: resuming with the same thread_id continues and finishes correctly
```
...
def risky_action(state):
    approval = interrupt({"question": f"Approve this risky action for task: {state['task']}?"})
    if not approval:
...
nodes.py:
    ...
    function act(state: AgentState) -> dict:
        ...
        never swallow a tool error silently -- catch it, record it in scratchpad, re-raise or route to a clear failure state

    function risky_action(state: AgentState) -> dict:
        approval = interrupt({"question": f"Approve this risky action for task: {state['task']}?"})
        if not approval: return final_answer explaining it was rejected, approved=False
        otherwise: do the placeholder risky thing, return final_answer, approved=True
...
test_graph.py:
    ...
    result = graph.invoke({"task": "...", "scratchpad": [], "next_action": "", "final_answer": "", "approved": False}, config=config)
...
graph.py:
    function build_graph(checkpointer=None) -> CompiledStateGraph:
        ...
        ... same node/edge wiring as Intermediate ...
        return builder.compile(checkpointer=checkpointer)
...
nodes.py -- risky_action, now with a real rejection path:
    function risky_action(state):
        ...
        if approval is not True:
            # explicit rejection, not a crash and not treated as silent approval
            return {"final_answer": "Risky action was not approved.", "approved": False}
        try:
...
Fill in the rejection path and the rest of `test_graph.py` yourself, then compare all 3 of your finished versions against the [Solution](#solution).

**Difference between Basic, Intermediate, and Advanced:** Basic's pseudocode gets one working risky-action node with an approval path. Intermediate wires the complete graph across all 4 files, with a real conditional edge mapping (including a direct route to `END`) and a real test file covering all 3 required behaviors. Advanced makes `build_graph()` accept its checkpointer instead of always creating its own, and gives `risky_action` a genuine 3-way outcome (approved / rejected / failed) instead of only handling the happy path — both of which matter the moment this skeleton gets imported and reused by Document 10, exactly as the Build Task's Goal says it will be.
...
def think(state: AgentState) -> dict:
    ...
    if action == "risky":
        return {"scratchpad": ["Thought: this needs approval"], "next_action": "risky"}
...
result1 = graph.invoke(
    {"task": "What is 12 * 7?", "scratchpad": [], "next_action": "", "final_answer": "", "approved": False},
    config=config1,
)
...
config2 = {"configurable": {"thread_id": "test-2"}}
paused = graph.invoke(
    {"task": "do a risky thing", "scratchpad": [], "next_action": "", "final_answer": "", "approved": False},
    config=config2,
)
...
def act(state: AgentState) -> dict:
    try:
        ...
        # never swallow it -- put the failure directly in state, where think() and
        # anything reading the final result can actually see what went wrong
        return {"scratchpad": [f"Observation: TOOL FAILED: {e}"], "next_action": "finish", "final_answer": f"Tool failed: {e}"}
    return {"scratchpad": [f"Observation: {result}"]}
...
assert "__interrupt__" not in result1, "should never pause on a task with no risky action"
...
assert "__interrupt__" in paused, "should pause before the risky action runs"
...
#### Approach 1 — a parameterized `build_graph`, ready for Document 10
...
def build_graph(checkpointer=None):
    ...
    Pass a checkpointer explicitly (e.g. a persisted one) for anything beyond
    quick local testing -- a fresh MemorySaver() is only created if none is given.
    """
    if checkpointer is None:
        checkpointer = MemorySaver()
...
print("All test_graph.py assertions passed, using an explicitly supplied checkpointer.")
```
**Expected output:**
```
All test_graph.py assertions passed, using an explicitly supplied checkpointer.
```

#### Approach 2 — swapping in a real persisted checkpointer, with zero other changes
...
```
Nothing in `graph.py`, `nodes.py`, or `state.py` changed — `build_graph(checkpointer=...)` from Approach 1 is exactly what makes this swap possible with a one-line change at the call site.

**Difference from Intermediate, and between these 2 Advanced approaches:** Intermediate's `build_graph()` always owns its own `MemorySaver()`, so every caller shares whatever instance was created inside `graph.py` — fine for one script, fragile the moment two different callers (a test file and Document 10's app) need genuinely independent graphs, or one of them needs a real persisted checkpointer. Approach 1 fixes that by accepting a checkpointer as a parameter. Approach 2 doesn't change any code at all — it's proof that Approach 1's one change is exactly the seam Document 10 needs: swap `MemorySaver()` for `SqliteSaver` (or a real database-backed saver in production) at the call site, with the graph skeleton itself untouched.

**Which one should you actually write?** Basic Version already satisfies every literal Build Task requirement — ship it first if you're short on time. Move to Intermediate's error-surfacing in `act` and the explicit rejection test as soon as you're done with the happy path — the "no node may quietly swallow an error" constraint is explicit in the Goal, and a human-in-the-loop gate genuinely needs a tested rejection path, not just an approval one. Reach for Advanced's parameterized `build_graph()` before you consider this skeleton actually done — the Build Task's whole stated purpose is that Document 10 imports and builds on it, and a hard-coded `MemorySaver()` inside `graph.py` is exactly the kind of thing that quietly breaks the moment a second caller needs its own checkpointer.
