...
**Story — `search_tool_integration_practice.py`:** this is the exact conditional edge the Build Task needs at its very first fork — should this task even touch the search tool, decided by the graph itself, before the model ever reasons about it. **If not:** the Build Task's router node would be the first place you ever wrote a real `should_search` function, with no smaller version to trust.
...
def should_search(state):
    ...
    prompt = (
        ...
        "in a document set? Answer only yes or no.\n\nQuestion: " + task
    )
...
### Approach 3 — a hybrid: cheap check first, model call only when ambiguous

**Story:** a keyword list and a single model call each fail in a different, predictable direction — keywords miss unexpected phrasings, a model call costs one extra round trip on every task, even the 90% that were obvious either way. **If not:** the Build Task's router would either misroute real search-needing questions or pay for a model call on every single task, including the ones any keyword list already handles for free.
...
### Approach 4 — logging every routing decision, for later review

**Story:** a hybrid router is still a guess, and guesses are worth checking — this keeps Approach 3's logic unchanged, but records which branch fired and why, so misroutes can be found and fixed later instead of going unnoticed. **If not:** "which cases are being misrouted" would stay a question you could only answer by inspection, never by evidence.
...
**Difference from Approach 1, and between Approaches 3/4:** Approach 1/2 each commit to one strategy for every task — either always the cheap check (missing some real search-needing questions) or always the model call (correct more often, but paying the cost and latency on every task, including the 90% that were obvious either way). Approach 3 fixes that by only paying for the model call on the genuinely ambiguous cases. Approach 4 doesn't change the routing decision at all — it adds a record of *why* each decision was made, which is what actually lets you improve the keyword list or the conversational heuristic later, based on real misroutes instead of guessing at them.

**Which one should you actually write?** Start with Approach 1 (keywords only) — it's free, instant, and good enough while you're building and testing the rest of the graph. Move to Approach 3's hybrid once you've actually seen the keyword version misroute a real question in testing — don't add the model-call fallback speculatively before you've confirmed you need it. Add Approach 4's logging once this router is handling real, varied traffic (not just your own test questions) — that's the point where "which cases are being misrouted" stops being something you can answer by inspection and becomes something you need evidence for.
