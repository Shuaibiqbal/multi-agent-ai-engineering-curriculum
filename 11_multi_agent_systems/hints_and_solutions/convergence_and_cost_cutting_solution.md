...
print("tokens by stage would need per-stage breakdown -- see")
print("Intermediate for that")
print("total:", report["total_tokens"], "tokens,")
print(report["total_seconds"], "seconds")
```
...
```
Couldn't converge after 3 attempts: draft must be under 10 words and
cite 3 sources; neither was met
```
...
def find_most_expensive(profile: dict) -> str:
    ...
    for stage, result in profile.items():
        if result.tokens > highest_tokens:
            highest_tokens = result.tokens
            most_expensive_stage = stage
    return most_expensive_stage
...
def research_shorter_prompt(topic: str) -> StageResult:
    ...
    prompt = f"List the top 3 causes of {topic}."  # shorter than the original
...
print(f"before: {before.tokens}t, after: {after.tokens}t")
print(f"saved: {saved_pct:.0f}%")
```
**Expected output** (exact numbers vary by run):
```
before: 210t, after: 152t
saved: 28%
```

**Difference from Basic:** Part 1's critic gives a specific, real reason instead of nothing — "under 10 words and cites 3 sources" is a concrete, genuinely unsatisfiable bar rather than a bare `passed = False`, and the final report explains *why* it failed, not just that it did. Part 2 now has a real per-stage breakdown (not just a total) that correctly identifies which stage to target, plus one actual cut (a shorter prompt) with a specific measured saving. Neither part yet proves the cut is safe on more than a single manual check, or shows whether the critic's rejection reason ever changes across attempts — that's what Approach 2 adds.
...
def build_report(result: dict) -> str:
    ...
    if len(unique_reasons) == 1:
        pattern = "the same reason every time -- criteria are likely impossible"
    else:
        pattern = "a different reason each time -- criteria may be inconsistent"

    attempts = result["revision_count"]
    lines = [f"Couldn't converge after {attempts} attempts ({pattern}):"]
...
```
**Expected output** (first line shown wrapped onto 2 lines just to fit the page — really one line of output):
```
Couldn't converge after 3 attempts (the same reason every time -- criteria
...
def compare_before_after(test_inputs: list[str]) -> None:
    ...
    tokens_saved = before_total_tokens - after_total_tokens
    saved_pct = tokens_saved / before_total_tokens * 100
    print(f"tested {len(test_inputs)} inputs")
    print(f"tokens before: {before_total_tokens}, after: {after_total_tokens}")
    print(f"saved: {saved_pct:.0f}%")
    if mismatches:
        print(f"POSSIBLE QUALITY DROP on {len(mismatches)} input(s):")
        print(mismatches)
    else:
...
**Difference between Approach 1 and Approach 2:** Approach 1's Part 1 gives one real reason but throws it away after each attempt, so the final report can't say whether the failure pattern was consistent or not. Approach 1's Part 2 measures one cut on one input and calls it done. Approach 2 turns both single manual checks into systematic ones — Part 1's full history turns "it failed" into "it failed *this specific, diagnosable way*," and Part 2's 5-input before/after comparison turns "it's cheaper" into "it's cheaper, and here's the proof nothing broke, or here's exactly which input needs a closer look."

**Which one should you actually write?** For any revision loop with a real hard limit — which every generator↔critic pattern needs, per this document's Core Concepts — Approach 2's full rejection history is worth keeping, since the cost of storing a list of strings is negligible next to the cost of an on-call engineer re-running a failed pipeline from scratch just to find out why it failed. For a cost cut, never ship one you've only checked against a single example — Approach 2's fixed test-set comparison is the minimum bar, and it's the exact same discipline `sequential_measure` and `supervisor_compare` already trained: don't trust a number you haven't measured against a real, repeated test set. If you're short on time, Approach 1's single-input check is acceptable for a quick local experiment, but re-run the full 5-input comparison before anything reaches Project 4's real pipeline.
