# Project 8 (Bonus) — MCPBridge-ReAct-Agent-That-Discovers-And-Uses-MCP-Tools-At-Runtime
...
> Not part of the numbered Doc01-19 project arc — a bonus/portfolio project pairing directly with [06_tools_function_calling](../06_tools_function_calling/)'s MCP Core Concepts and [07_ai_agents](../07_ai_agents/)'s ReAct-loop Core Concepts. Read both first if you haven't already — this file has the full build spec, not a pointer elsewhere.
...
*Project: **MCPBridge-ReAct-Agent-That-Discovers-And-Uses-MCP-Tools-At-Runtime** — Step 1 of 4: Connect to One MCP Server and List Its Tools*
...
```
project_8_mcp_client_agent/
├── example_server.py       → (only if you're not using Project 7's or a public server) minimal FastMCP server, a few tools
...
*Project: **MCPBridge-ReAct-Agent-That-Discovers-And-Uses-MCP-Tools-At-Runtime** — Step 2 of 4: A Real ReAct Agent That Calls a Discovered Tool*
...
*Project: **MCPBridge-ReAct-Agent-That-Discovers-And-Uses-MCP-Tools-At-Runtime** — Step 3 of 4: Multiple MCP Servers at Once, With Tool Namespacing*
...
*Project: **MCPBridge-ReAct-Agent-That-Discovers-And-Uses-MCP-Tools-At-Runtime** — Step 4 of 4: A Fallback When a Server Is Unreachable, and a Full Working Demo*
...
**Final Deliverable:** **MCPBridge-ReAct-Agent-That-Discovers-And-Uses-MCP-Tools-At-Runtime** — a single ReAct agent that connects to one or more real MCP servers, discovers their tools at runtime instead of hardcoding any of them, routes namespaced tool calls to the right server, and keeps working on whatever tools remain available when a server is unreachable.
...
- [ ] The ReAct loop (Step 2) reuses Doc07's think→act→observe shape, hand-built, not `AgentExecutor`
